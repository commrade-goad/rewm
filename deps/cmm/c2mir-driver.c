...
static const char *expand_tag = "#expand";
static size_t expand_tag_len = 7;
...
static void init_options (int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    if (strcmp (argv[i], "-expand-tag") == 0) {
      if (i + 1 >= argc) {
        fprintf (stderr, "-expand-tag requires an argument -- goodbye\n");
        exit (1);
      }
      expand_tag = argv[++i];
      expand_tag_len = strlen (expand_tag);
      continue;
...
static void db_init (dyn_buf_t *b) {
  b->size = 256;
  b->len = 0;
  b->data = MIR_malloc (&default_alloc, b->size);
  b->data[0] = 0;
}

static void db_append_n (dyn_buf_t *b, const char *s, size_t n) {
  if (n == 0) return;
  if (b->len + n + 1 > b->size) {
      size_t old_size = b->size;
    while (b->len + n + 1 > b->size) b->size *= 2;
    b->data = MIR_realloc (&default_alloc, b->data, old_size, b->size);
  }
...
static void db_append (dyn_buf_t *b, const char *s) { db_append_n (b, s, strlen (s)); }
...
static void emit_literal_text (dyn_buf_t *body, const char *text, size_t len) {
  ...
  for (size_t i = 0; i < len; i++) {
    ...
    switch (c) {
    case '\\': db_append (body, "\\\\"); break;
    case '"': db_append (body, "\\\""); break;
    case '\n': db_append (body, "\\n"); break;
    case '\r': db_append (body, "\\r"); break;
    case '\t': db_append (body, "\\t"); break;
    case '\0': break; /* drop embedded NULs */
 ...
 * re-anchor error reporting when template content gets moved/collapsed
 * around during expansion (see emit_line_directive below). */
static int line_number_at (const uint8_t *code, size_t offset) {
  int line = 1;
...
/* Append a `#line <n> "<filename>"` directive to `b`. This tells the
 * compiler's diagnostics (and any further #line-aware tooling) which line
 * of the *original* template file the text that follows corresponds to,
 * even though it has been physically relocated/collapsed in the generated
 * C file. Filename is escaped minimally (backslash/quote) since it can
 * contain backslashes on Windows. */
static void emit_line_directive (dyn_buf_t *b, int line, const char *filename) {
  char num[32];
  ...
  for (const char *s = filename; *s != 0; s++) {
    if (*s == '\\' || *s == '"') db_append_n (b, "\\", 1);
    db_append_n (b, s, 1);
  }
  db_append (b, "\"\n");
}

/* Core transform: turn a raw template file into a compilable C file.
 * Only call this when contains_expand() is true. Returned buffer is
 * MIR_malloc'd; caller should register it in `allocated` for cleanup. */
static uint8_t *expand_templates (const uint8_t *code, size_t code_len, size_t *out_len,
                                   const char *filename) {
  dyn_buf_t prelude, body, out;
  const uint8_t *p = code, *end = code + code_len;
  const uint8_t *literal_start = p;
  ...
  while (p < end) {
    if ((size_t) (end - p) >= expand_tag_len
        && memcmp (p, expand_tag, expand_tag_len) == 0
        && !is_ident_char (p == code ? 0 : p[-1])) {

      const uint8_t *q = p + expand_tag_len;
      int is_prelude_block = 0;

      // Skip whitespace before checking for the modifier separator ':'
      ...
      if (q < end && *q == ':') {
        ...
        // Look for either "prelude" or "global" keywords explicitly
        if ((size_t)(end - q) >= 7 && memcmp (q, "prelude", 7) == 0) {
          is_prelude_block = 1;
          q += 7;
        } else if ((size_t)(end - q) >= 6 && memcmp (q, "global", 6) == 0) {
          is_prelude_block = 1;
          q += 6;
        }
      }
      ...
      while (q < end && isspace ((unsigned char) *q)) q++;
      if (q < end && *q == '{') {
        /* flush the literal text seen before this block */
        emit_literal_text (&body, (const char *) literal_start, (size_t) (p - literal_start));

        /* brace-match the block body, skipping strings/chars/comments */
        const uint8_t *b = q + 1;
        const uint8_t *block_start = b;
        int depth = 1;
        while (b < end && depth > 0) {
          if (*b == '"' || *b == '\'') {
            uint8_t quote = *b++;
        ...
        {
          const uint8_t *block_end = b; /* at closing '}' */
          /* Skip the leading newline(s) left over right after the opening '{' (e.g.
           * "@@ {\n  code..."). This is pure formatting, not content -- and critically,
           * c2mir does not advance its line counter across a blank line that immediately
           * follows a #line directive, so keeping it here would throw the numbering off
           * by one. Trimming it means the #line we emit points exactly at the first real
           * line of code, and everything after tracks correctly via real newlines. */
          const uint8_t *content_start = block_start;
          while (content_start < block_end && (*content_start == '\n' || *content_start == '\r'))
            content_start++;
          size_t blen = (size_t) (block_end - content_start);
          int block_line = line_number_at (code, (size_t) (content_start - code));

          // Route the block token stream cleanly based on our descriptive modifier check
          if (is_prelude_block) {
            emit_line_directive (&prelude, block_line, filename);
            db_append_n (&prelude, (const char *) content_start, blen);
            db_append (&prelude, "\n");
          } else {
            emit_line_directive (&body, block_line, filename);
            db_append_n (&body, (const char *) content_start, blen);
            db_append (&body, "\n");
          }
        }

        p = (b < end) ? b + 1 : end; /* resume after the closing '}' */
        literal_start = p;
        continue;
      }
    }
  ...
  /* flush trailing literal text after the last block */
  ...
  MIR_free (&default_alloc, prelude.data);
  MIR_free (&default_alloc, body.data);
  ...
  for (i = 0, options.module_num = 0;; i++, options.module_num++) {
    ...
    if ((bin_p = len >= 5 && strcmp (curr_input.input_name + len - 5, ".bmir") == 0)
      ...
      if (curr_input.code != NULL && template_mode == TRUE ) {
        ...
        VARR_PUSH (void_ptr_t, allocated, expanded); /* freed at shutdown like everything else */
        if (curr_input.code_container != NULL) {
          VARR_DESTROY (uint8_t, curr_input.code_container);
          curr_input.code_container = NULL;
        }
        curr_input.code = expanded;
        curr_input.code_len = new_len;
      }
      /* >>> END NEW <<< */
