...
static htab_hash_t cstr_hash (char_ptr_t s, void *arg MIR_UNUSED) {
  return (htab_hash_t) mir_hash (s, strlen (s), 0x42);
}
static int cstr_eq (char_ptr_t a, char_ptr_t b, void *arg MIR_UNUSED) {
  return strcmp (a, b) == 0;
}
...
struct pre_ctx {
  HTAB (char_ptr_t) * included_files;
...
#define included_files pre_ctx->included_files
...
static void pre_init (c2m_ctx_t c2m_ctx) {
  ...
  HTAB_CREATE (char_ptr_t, included_files, alloc, 64, cstr_hash, cstr_eq, NULL);
...
static void pre_finish (c2m_ctx_t c2m_ctx) {
  ...
  if (included_files != NULL) HTAB_DESTROY (char_ptr_t, included_files);
...
static void add_include_stream (c2m_ctx_t c2m_ctx, const char *fname, const char *content,
  ...
  assert (fname != NULL);
  if (HTAB_DO (char_ptr_t, included_files, fname, HTAB_FIND, found)) return;
  ...
  HTAB_DO (char_ptr_t, included_files, fname, HTAB_INSERT, found);
}
...
static void check_pragma (c2m_ctx_t c2m_ctx, token_t t, VARR (token_t) * tokens) {
  ...
  if (i + 1 == tokens_len && tokens_arr[i]->code == T_ID
    ...
    return;  /* already tracked automatically by add_include_stream */
  }
...
static void pre (c2m_ctx_t c2m_ctx) {
  ...
  pptokens_num = 0;
  HTAB_CLEAR (char_ptr_t, included_files);
...
D (initializer_list) {
  ...
  list = new_node (c2m_ctx, N_LIST);
  if (C ('}')) {
    node_t zero = new_i_node (c2m_ctx, 0, curr_token->pos);
    node_t des = new_node (c2m_ctx, N_LIST);
    op_append (c2m_ctx, list, new_node2 (c2m_ctx, N_INIT, des, zero));
    return list;
  }
  for (;;) { /* designation */
...
D (stmt) {
  ...
  } else if (MP (T_DEFER, pos)) { /* extension: defer-statement */
    P (stmt);
    op1 = r;
    r = new_pos_node2 (c2m_ctx, N_DEFER, pos, l, op1);
...
static void check (c2m_ctx_t c2m_ctx, node_t r, node_t context) {
  ...
  case N_DEREF_FIELD: {
    ...
    if (t1->mode == TM_PTR && (r->code == N_DEREF_FIELD || r->code == N_FIELD)) {
        /* auto-deref extension: `.` behaves like `->` when the base expr is a pointer.
           Promote the node so all downstream code (which only ever checks
           r->code == N_DEREF_FIELD) treats it exactly like a real `->`. */
...
/* ================= `defer` statement desugaring ========================
   `defer <stmt>;` (or `defer { ... }`) registers <stmt> to run when the
   enclosing `{ }` block is left: by falling off its end, or by an early
   `return`/`break`/`continue` that leaves it.  Defers in the same block
   run in LIFO order, like Zig/Go.

   This is implemented as a pure AST-to-AST rewrite that runs right after
   parsing and before context checking, so the rest of the compiler never
   sees an N_DEFER node.

   Known limitations (rejected with a normal compile error, or otherwise
   documented rather than silently mishandled):
   - `defer` must be a direct statement inside a `{ }` block; it cannot be
     the (unbraced) single-statement body of `if`/`while`/`for`/`do`.
   - `goto` is rejected in a scope where a `defer` is (or may be) pending,
     since jumping into/out of such a scope cannot be handled by a static
     duplication of the deferred code.
   - a deferred statement's own code is not itself subject to the outer
     scopes' defers; e.g. a `return` written directly inside a deferred
     block only returns, it does not re-trigger other pending defers.
   - because a deferred statement can be duplicated at more than one exit
     point, it should not contain a named label used as a `goto` target.
*/

struct defer_scope {
  VARR (node_t) * defers; /* pending deferred statement templates, declaration order */
  struct defer_scope *up; /* enclosing block's defer scope, or NULL */
};
typedef struct defer_scope *defer_scope_t;

struct loop_ctx {
  defer_scope_t scope_at_entry; /* defer scope in effect right before the loop/switch */
};
typedef struct loop_ctx *loop_ctx_t;

static int defer_leaf_node_p (node_code_t code) {
  switch (code) {
  ...
  case N_ID: return TRUE;
  default: return FALSE;
  }
}

/* Deep-copy a statement (sub)tree so it can be safely spliced into more than
   one place -- AST nodes here can only ever belong to a single parent. */
static node_t defer_deep_copy (c2m_ctx_t c2m_ctx, node_t n) {
  node_t r, child;

  if (n == NULL) return NULL;
  if (defer_leaf_node_p (n->code)) return copy_node (c2m_ctx, n);
  r = new_pos_node (c2m_ctx, n->code, POS (n));
  /* Use op_append, not op_flat_append: children here are exact structural
     operands (e.g. a statement's own, possibly-empty, N_LIST-coded labels
     child) and must be preserved as single children, never flattened. */
  for (child = NL_HEAD (n->u.ops); child != NULL; child = NL_NEXT (child))
    op_append (c2m_ctx, r, defer_deep_copy (c2m_ctx, child));
  return r;
}

static int defer_scope_pending_p (defer_scope_t scope) {
  for (; scope != NULL; scope = scope->up)
    if (VARR_LENGTH (node_t, scope->defers) > 0) return TRUE;
  return FALSE;
}

/* Append copies of the deferred statements pending between `from` (inclusive)
   and `stop_before` (exclusive) onto `new_list`, in LIFO order. */
static void defer_emit_pending (c2m_ctx_t c2m_ctx, defer_scope_t from, defer_scope_t stop_before,
                                 node_t new_list) {
  size_t i;

  for (; from != NULL && from != stop_before; from = from->up)
    for (i = VARR_LENGTH (node_t, from->defers); i > 0; i--)
      op_flat_append (c2m_ctx, new_list, defer_deep_copy (c2m_ctx, VARR_GET (node_t, from->defers, i - 1)));
}

static node_t defer_transform_block (c2m_ctx_t c2m_ctx, node_t block, defer_scope_t up_scope,
                                      loop_ctx_t break_ctx, loop_ctx_t continue_ctx);
static node_t defer_transform_single (c2m_ctx_t c2m_ctx, node_t item, defer_scope_t scope,
                                       loop_ctx_t break_ctx, loop_ctx_t continue_ctx);
static void defer_transform_append (c2m_ctx_t c2m_ctx, node_t item, node_t new_list,
                                     defer_scope_t scope, loop_ctx_t break_ctx,
                                     loop_ctx_t continue_ctx);

/* Replace child `old` of `parent` with `new_ch` (no-op if they are the same node). */
static void defer_replace_child (node_t parent, node_t old, node_t new_ch) {
  if (old == new_ch) return;
  DLIST_INSERT_AFTER (node_t, parent->u.ops, old, new_ch);
  NL_REMOVE (parent->u.ops, old);
}

/* Detach `child` from `parent`'s op list, transform it as a single-statement
   slot, and splice the result back in at the same position. The child must
   be detached *before* transforming it: transforming may splice it (or
   nodes derived from it) into an unrelated list, which would otherwise
   corrupt `parent`'s list out from under it (a node can only ever belong to
   one parent's op list at a time). */
static void defer_transform_child (c2m_ctx_t c2m_ctx, node_t parent, node_t child,
                                    defer_scope_t scope, loop_ctx_t break_ctx,
                                    loop_ctx_t continue_ctx) {
  node_t prev = NL_PREV (child);
  node_t new_child;

  NL_REMOVE (parent->u.ops, child);
  new_child = defer_transform_single (c2m_ctx, child, scope, break_ctx, continue_ctx);
  if (prev != NULL)
    DLIST_INSERT_AFTER (node_t, parent->u.ops, prev, new_child);
  else
    NL_PREPEND (parent->u.ops, new_child);
}
...
static void defer_transform_append (c2m_ctx_t c2m_ctx, node_t item, node_t new_list,
  ...
  switch (item->code) {
  case N_BLOCK:
    op_flat_append (c2m_ctx, new_list, defer_transform_block (c2m_ctx, item, scope, break_ctx, continue_ctx));
    break;
  case N_IF:
    defer_transform_child (c2m_ctx, item, NL_EL (item->u.ops, 2), scope, break_ctx, continue_ctx);
    defer_transform_child (c2m_ctx, item, NL_EL (item->u.ops, 3), scope, break_ctx, continue_ctx);
    op_flat_append (c2m_ctx, new_list, item);
    break;
  case N_WHILE: case N_DO: {
    struct loop_ctx lc;
    lc.scope_at_entry = scope;
    defer_transform_child (c2m_ctx, item, NL_EL (item->u.ops, 2), scope, &lc, &lc);
    op_flat_append (c2m_ctx, new_list, item);
    break;
  }
  case N_FOR: {
    struct loop_ctx lc;
    lc.scope_at_entry = scope;
    defer_transform_child (c2m_ctx, item, NL_EL (item->u.ops, 4), scope, &lc, &lc);
    op_flat_append (c2m_ctx, new_list, item);
    break;
  }
  case N_SWITCH: {
    struct loop_ctx lc;
    lc.scope_at_entry = scope;
    defer_transform_child (c2m_ctx, item, NL_EL (item->u.ops, 2), scope, &lc, continue_ctx);
    op_flat_append (c2m_ctx, new_list, item);
    break;
  }
  case N_RETURN:
    defer_emit_pending (c2m_ctx, scope, NULL, new_list);
    op_flat_append (c2m_ctx, new_list, item);
    break;
  case N_BREAK:
    if (break_ctx != NULL) defer_emit_pending (c2m_ctx, scope, break_ctx->scope_at_entry, new_list);
    op_flat_append (c2m_ctx, new_list, item);
    break;
  case N_CONTINUE:
    if (continue_ctx != NULL) defer_emit_pending (c2m_ctx, scope, continue_ctx->scope_at_entry, new_list);
    op_flat_append (c2m_ctx, new_list, item);
    break;
  case N_GOTO: case N_INDIRECT_GOTO:
    if (defer_scope_pending_p (scope))
      error (c2m_ctx, POS (item),
             "'goto' is not supported in a scope where a 'defer' is active");
    op_flat_append (c2m_ctx, new_list, item);
    break;
  case N_DEFER:
    error (c2m_ctx, POS (item), "'defer' must be a direct statement inside a { } block");
    break; /* drop it -- error recovery */
  default: op_flat_append (c2m_ctx, new_list, item); break;
  }
}

/* Transform a single-statement slot (if/while/do/for/switch body). Always
   returns exactly one node, wrapping in a synthetic block if the statement
   needed to expand into more than one (e.g. defers injected before a
   `return`). */
static node_t defer_transform_single (c2m_ctx_t c2m_ctx, node_t item, defer_scope_t scope,
                                       loop_ctx_t break_ctx, loop_ctx_t continue_ctx) {
  node_t tmp_list;

  if (item->code == N_BLOCK) return defer_transform_block (c2m_ctx, item, scope, break_ctx, continue_ctx);
  if (item->code == N_IGNORE) return item;
  if (item->code == N_DEFER) {
    error (c2m_ctx, POS (item), "'defer' must be a direct statement inside a { } block");
    return new_node (c2m_ctx, N_IGNORE);
  }
  tmp_list = new_node (c2m_ctx, N_LIST);
  defer_transform_append (c2m_ctx, item, tmp_list, scope, break_ctx, continue_ctx);
  if (NL_HEAD (tmp_list->u.ops) != NULL && NL_HEAD (tmp_list->u.ops) == NL_TAIL (tmp_list->u.ops))
    return NL_HEAD (tmp_list->u.ops);
  return new_pos_node2 (c2m_ctx, N_BLOCK, POS (item), new_node (c2m_ctx, N_LIST), tmp_list);
}

/* Transform a full `{ }` block: rewrite its statement list, removing
   N_DEFER items (registering them) and appending their LIFO cleanup code
   at the natural end of the block. */
static node_t defer_transform_block (c2m_ctx_t c2m_ctx, node_t block, defer_scope_t up_scope,
                                      loop_ctx_t break_ctx, loop_ctx_t continue_ctx) {
  MIR_alloc_t alloc = c2m_alloc (c2m_ctx);
  struct defer_scope scope;
  node_t stmt_list = NL_EL (block->u.ops, 1);
  node_t new_list = new_node (c2m_ctx, N_LIST);
  node_t item, next_item;
  size_t i;

  scope.up = up_scope;
  VARR_CREATE (node_t, scope.defers, alloc, 4);
  for (item = NL_HEAD (stmt_list->u.ops); item != NULL; item = next_item) {
    next_item = NL_NEXT (item);
    NL_REMOVE (stmt_list->u.ops, item);
    if (item->code == N_DEFER) {
      node_t labels = NL_HEAD (item->u.ops);
      node_t body = NL_EL (item->u.ops, 1);
      if (NL_HEAD (labels->u.ops) != NULL)
        error (c2m_ctx, POS (item), "labels are not allowed on a 'defer' statement");
      NL_REMOVE (item->u.ops, body);
      /* A deferred statement's own control flow does not chain into the
         outer scopes (see function-level comment above). */
      body = defer_transform_single (c2m_ctx, body, NULL, NULL, NULL);
      VARR_PUSH (node_t, scope.defers, body);
      continue;
    }
    defer_transform_append (c2m_ctx, item, new_list, &scope, break_ctx, continue_ctx);
  }
  for (i = VARR_LENGTH (node_t, scope.defers); i > 0; i--)
    op_flat_append (c2m_ctx, new_list, defer_deep_copy (c2m_ctx, VARR_GET (node_t, scope.defers, i - 1)));
  VARR_DESTROY (node_t, scope.defers);
  defer_replace_child (block, stmt_list, new_list);
  return block;
}

/* Safety net: after the rewrite above, no N_DEFER node should remain
   anywhere reachable from a function body. If one slipped through (e.g.
   nested inside a GNU statement-expression, which the rewrite above does
   not look inside), report it here instead of letting the rest of the
   compiler crash on an unrecognized node. */
static void defer_check_none_left (c2m_ctx_t c2m_ctx, node_t n) {
  node_t child;

  if (n == NULL || defer_leaf_node_p (n->code)) return;
  if (n->code == N_DEFER) {
    error (c2m_ctx, POS (n), "'defer' is not supported in this context");
    return;
  }
  for (child = NL_HEAD (n->u.ops); child != NULL; child = NL_NEXT (child))
    defer_check_none_left (c2m_ctx, child);
}

static void transform_defers (c2m_ctx_t c2m_ctx, node_t module) {
  node_t list = NL_HEAD (module->u.ops); /* N_MODULE has a single N_LIST operand */
  node_t n;

  for (n = NL_HEAD (list->u.ops); n != NULL; n = NL_NEXT (n))
    if (n->code == N_FUNC_DEF) {
      node_t body = NL_TAIL (n->u.ops); /* last operand: compound_stmt */
      defer_transform_block (c2m_ctx, body, NULL, NULL, NULL);
    }
  for (n = NL_HEAD (list->u.ops); n != NULL; n = NL_NEXT (n)) defer_check_none_left (c2m_ctx, n);
}

/* ================= end of `defer` statement desugaring ================== */
...
int c2mir_compile (MIR_context_t ctx, struct c2mir_options *ops, int (*getc_func) (void *),
  ...
  if (!c2m_options->prepro_only_p) {
    ...
    if (n_errors == 0) transform_defers (c2m_ctx, r);
